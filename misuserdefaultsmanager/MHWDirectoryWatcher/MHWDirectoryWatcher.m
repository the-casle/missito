...
- (BOOL)startWatching
{
    ...
    self.deleteSource = dispatch_source_create(DISPATCH_SOURCE_TYPE_VNODE, // Monitors a file descriptor
                           fd, // our file descriptor
                           DISPATCH_VNODE_DELETE, // The file-system object data changed.
                           queue); // the queue to dispatch on
    ...
    dispatch_source_set_event_handler(self.deleteSource, ^{
    ...
    dispatch_source_set_cancel_handler(self.deleteSource, cleanup);
    
    // Sources are create in suspended state, so resume it
    dispatch_resume(self.source);
    dispatch_resume(self.deleteSource);
...
- (NSString *)directoryMetadata
{
    NSString *fileName = self.watchedPath;
    NSDictionary *fileAttributes = [[NSFileManager defaultManager] attributesOfItemAtPath:fileName
                                                                                    error:nil];
...
{
    NSString *directoryMetadata = [self directoryMetadata];
...
- (void)pollDirectoryForChangesWithMetadata:(NSString *)oldDirectoryMetadata
{
    
    NSString *newDirectoryMetadata = [self directoryMetadata];
    
    // Check if metadata has changed
    self.directoryChanging = ![newDirectoryMetadata isEqualToString:oldDirectoryMetadata];
